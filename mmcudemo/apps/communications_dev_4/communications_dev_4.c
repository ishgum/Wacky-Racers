...
void motor_poll(void) {
	if (motors.throttle_update > 20) {
		if (motors.throttle > MOTOR_THROTTLE_STOP) {
			motors.throttle = motors.throttle - 1;
		}
		if (motors.throttle < MOTOR_THROTTLE_STOP) {
			motors.throttle = motors.throttle + 1;
		}
	...
	if (motors.throttle > MOTOR_THROTTLE_FORWARD) {
		motors.throttle = MOTOR_THROTTLE_FORWARD;
	}
	if (motors.throttle < MOTOR_THROTTLE_BACK) {
		motors.throttle = MOTOR_THROTTLE_BACK;
	}
	if (motors.turn > (MOTOR_TURN_RIGHT - 31)) {
		motors.turn = MOTOR_TURN_RIGHT;
	}
	if (motors.turn < (MOTOR_TURN_LEFT + 31)) {

...
#define HIGH_BIT 1300
#define LOW_BIT 660
#define TOLERANCE 150
#define START_BIT_TOL 800
#define IR_BUFFER 64

#define OFF_BUTTON 3772793023
#define UP_BUTTON 3772778233
#define DOWN_BUTTON 268288
#define LEFT_BUTTON 3772819033
#define RIGHT_BUTTON  3772794553
#define CENTRE_BUTTON 3772782313
...
void irInterruptHandler (void) {
	if (writeArray) {
		...
		if (abs(START_BIT - difference) < START_BIT_TOL) {
			if (startFound)
			{
				writeArray = 0;
				readArray = 1;
    ...
    while (1)
    {
		...
		if (usb_cdc_update()){
			if (readArray) {
				...
				for (; i < IR_BUFFER; i++)
				{
					unsigned long long val = differenceArray[i];
					if (val != 0)
						printf("%llu\n\r", val );
				}
			}
		...
		*/
		...
		if (writeArray && startFound)
		{
			difference = (tc_counter_get (tc) - prev_time);
			if (difference > (START_BIT + START_BIT_TOL))
			{
				writeArray = 0;
				readArray = 1;
				startFound = 0;

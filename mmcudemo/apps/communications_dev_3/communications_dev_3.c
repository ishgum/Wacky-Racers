...
typedef struct motor_struct_first {
	char throttle;
	char turn;
	signed char throttleDir;
	signed char turnDir;
	bool throttle_update;
	bool turn_update;
} motor_struct;
...
void process_ir_command (unsigned int ir_data) {
	...
	if (ir_data == BUTTON_FORWARD) {
		motors.throttle_update = 1;
		motors.throttleDir = 1;
	}
...
void motor_poll(void) {
	if (motors.throttle_update) {
		if (motors.throttle < 145 && motors.throttle > 11) {
			motors.throttle = motors.throttle + 10*motors.throttleDir;
		}
	}
	else {
		if (motors.throttle > MOTOR_THROTTLE_STOP) {
			motors.throttle = motors.throttle - 10;
		}
		if (motors.throttle < MOTOR_THROTTLE_STOP) {
			motors.throttle = motors.throttle + 10;
		}
	...
	if (motors.turn_update) {
		if (motors.turn < 224 && motors.turn > 31) {
			motors.turn = motors.turn + 30*motors.turnDir;
		}
	}
	else {
		if (motors.turn > MOTOR_TURN_CENTRE) {
			motors.turn = motors.turn - 30;
		}
		if (motors.turn < MOTOR_TURN_CENTRE) {
			motors.turn = motors.turn + 30;
		}
